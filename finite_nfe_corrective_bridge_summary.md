...
x_k
+
h\Big[v_0(x_k,t_k;c)+c_\phi(x_k,t_k,c,N)\Big],
\qquad
h=\frac1N,\quad t_k=\frac{k}{N}.
\]
...
The frozen endpoint is used as a **same-noise counterfactual baseline**. This gives lower-variance, more stable supervision than optimizing the corrected terminal loss directly.
...
F_\phi^N
=
G_{\phi,N-1}\circ\cdots\circ G_{\phi,0},
\]
...
G_{\phi,k}(x)
=
x+h\Big[v_0(x,t_k;c)+c_\phi(x,t_k,c,N)\Big],
\]

and then optimize the weights of this finite-time operator directly:
...
=
\arg\min_\phi
\mathcal L_{\mathrm{pair}}\!\left(F_\phi^N,F_0^N\right).
}
\]

Hence the method is **discretize first, then optimize the realized finite-NFE transport**.
...
## 4. An implicit flow-map / finite-time operator model
...
Thus terminal supervision automatically assigns credit over the whole trajectory.
...
not exact \(h\)-transform equivalence.

---

## 6. Mean displacement vs distribution deformation

To understand *how* the unrestricted corrective transport improves the task, we use a post-hoc counterfactual diagnostic.

For a fixed context and NFE, define the endpoint distributions
...
X_0=\mu_0+A_0,
\qquad
X_\phi=\mu_\phi+A_\phi,
\]

where \(A_0,A_\phi\) are centered anomalies.
...
- the **mean learned by the full corrective transport**, and
...
so pairwise differences, covariance, effective rank, and all translation-invariant diversity statistics are preserved exactly in the modeled state space.

Operationally, estimating \(\mu_\phi-\mu_0\) requires both frozen and corrected ensembles, so this is primarily a mechanism diagnostic rather than the preferred deployment method.
...
\text{full task gain}
=
\text{gain from conditional mean displacement}
+
\text{additional gain enabled by distribution deformation}
}
\]

with the caveat that nonlinear task metrics make the two effects non-additive in general.

The mean counterfactual

\[
X_{\mathrm{mean}}=\mu_\phi+A_0
\]
...
This gives two natural operating interpretations:

### Diversity-sensitive use

Use the full model for learning, but analyze or construct the mean-shifted base distribution

\[
\mu_\phi+A_0
\]

when preserving the base stochastic geometry is important.

### Fidelity-dominant use
...
1. **Paired objective**  
   Learn incremental utility relative to the same-noise frozen bridge.
...
3. **Implicit flow-map learning**  
   The model is parameterized as local velocity / score corrections but supervised at the terminal flow-map level.
...
5. **Mechanism decomposition**  
   Post-hoc mean transport
   \[
   X_0+(\mu_\phi-\mu_0)
   \]
   separates conditional mean displacement from centered distribution deformation.

6. **Fidelity-diversity trade-off**  
   Most gain comes from learned mean displacement; unrestricted deformation supplies further fidelity but can suppress diversity and coverage.

A concise description is:

> **We post-train a frozen generative bridge by optimizing its deployed finite-NFE flow map through a residual velocity/score field. Paired terminal supervision learns task-optimal transport rather than merely numerical correction. A simple same-context counterfactual decomposition shows that most downstream gain arises from conditional mean transport, while the remaining gain is associated with deformation of the centered generative distribution and the resulting fidelity-diversity trade-off.**
